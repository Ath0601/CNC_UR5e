...
def generate_launch_description():
    ...
    xacro_file = PathJoinSubstitution([pkg_share, 'urdf', 'ur5e_corrected_with_tcp.urdf.xacro'])
    ...
    spindle_ctrl_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['spindle_velocity_controller', '--controller-manager', '/controller_manager'],
        output='screen'
    )

    moveit_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([FindPackageShare('moveit2_config'), 'launch', 'move_group.launch.py'])
        ])
    )
